...
1. 在 `首页` 看“下一步”提示。
2. 点击 `1 创建示例项目`，或者使用 `使用自己的数据` 创建项目。
3. 如果使用自己的数据，先看 `上传预览`，确认三个文件都是 `可读取`。
4. 点击 `2 检查输入`。
5. 第一次运行时保持 `Dry run` 勾选，然后点击 `3 运行流程`。
6. 如果有输入检查问题，先根据提示修复。
7. 只有在 BioGeoBEARS 已安装时，才取消 `Dry run`。
8. 再次点击 `3 运行流程`，运行真实分析。
9. 点击 `4 生成报告` 生成报告。
10. 打开 `结果`，先看祖先重建图、模型比较表和事件统计。
11. 如果需要分享结果，点击 `生成结果压缩包`。
12. 如果需要别人帮你排查问题，点击 `生成诊断压缩包`。

## 5. 使用自己的数据

最简单的方法是在图形界面里使用 `使用自己的数据`。

需要准备三个文件：

- Newick 格式的系统树文件。
- 分布矩阵 CSV：每个物种或样本一行，每个地理区域一列。
- 区域信息 CSV：区域编号和区域名称。

上传三个文件后，先看界面里的 `上传预览` 表。如果某个文件显示 `需要检查`，先修正文件格式，再点击 `创建自己的分析项目`。
